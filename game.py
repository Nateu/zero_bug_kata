...
class Board:
    def __init__(self, size: int = 12, categories: List[str] = ["Pop", "Science", "Sports", "Rock"]) -> None:
...
class Game:
    ...
    def add_player(self, player_name: str) -> bool:
        if self.player_count() >= self.max_players:
            print(
                f"Max number of players is {self.max_players}."
            )
            return False
        ...
        print(
            f"{player_name} was added.\n" +
            f"They are player number {self.player_count()}."
        )
        return True
    ...
    def move_player_and_ask_question(self, roll: int) -> None:
        ...
        print(
            f"{self.get_current_player()}'s new location is {current_place}.\n" +
    ...
    def current_player_gains_coin(self) -> None:
        print(
            f"{self.get_current_player()} now has {self.get_current_player().add_coins()} Gold Coins."
        )

    def finish_turn(self) -> bool:
        if self.check_if_current_player_has_won():
            print (
                f"{self.get_current_player()} has won!!"
            )
    ...
    def incorrectly_answered(self) -> bool:
        print(
            f"Question was incorrectly answered.\n" +
            f"{self.get_current_player()} was sent to the penalty box."
        )
